...
from kairos.modules.schedulers.flow_match import FlowMatchScheduler, DMDFlowMatchScheduler 
...
class KairosEmbodiedPipeline_DMD(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, vram_management_enabled = False, use_unified_sequence_parallel=False):
        ...
        self.scheduler = DMDFlowMatchScheduler(shift=5, sigma_min=0.0, extra_one_step=True,
                                             exponential_shift=False, exponential_shift_mu=1.609)
        self.prompter = None
    ...
    def __call__(
        ...
        self.scheduler.set_timesteps(
            selected_sampling_time=self.selected_sampling_time, dynamic_shift_len=dynamic_shift_len, num_frames=latent_T)
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Inference
            if len(self.scheduler.timesteps) - progress_id<2:
                velocity_pred = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep)           
                ...
                # in euler solver last step, xo=xt
                inputs_shared["latents"] = x0
...
class WanVideoUnit_ShapeChecker(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, height, width, num_frames):
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("input_video", "height", "width", 'batch_size',"num_frames", "seed", "rand_device", "vace_reference_image"))

    def process(self, pipe: KairosEmbodiedPipeline_DMD, input_video, height, width, batch_size, num_frames, seed, rand_device, vace_reference_image):
        ...
        # print('gen noise: seed: {} ||  shape : {} || mean: {} std: {}'.format(seed, shape, noise.mean(), noise.std()))
        if vace_reference_image is not None:
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        if input_video is None:
            return {"latents": noise}
        pipe.load_models_to_device(["vae"])
        input_video = pipe.preprocess_video(input_video)
        # print(f'DBG VAE: {self.__class__.__name__} input_video: {len(input_video)}, noise: {noise.shape}, tiled:{tiled}, tile_size:{tile_size}, tile_stride:{tile_stride}, vace_reference_image:{vace_reference_image}')
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            ...
            input_params_posi={"prompt": "prompt", "positive": "positive", "drop_visual_token_in_vlm_probs": "drop_visual_token_in_vlm_probs"},
            input_params_nega={"prompt": "negative_prompt", "positive": "positive", "drop_visual_token_in_vlm_probs": "drop_visual_token_in_vlm_probs"},
            onload_model_names=("text_encoder",)
        )

    def process(self, pipe, prompt, positive, input_image=None, drop_visual_token_in_vlm_probs=False) -> dict:
        if positive is not None and not positive:
            input_image = None
        if drop_visual_token_in_vlm_probs:
            # print('TI2V drop_visual_token_in_vlm_probs !!! ')
            input_image = None

        pipe.load_models_to_device(self.onload_model_names)
        # print(f'DBG PROMPTER: {self.__class__.__name__} positive: {positive} prompt: {prompt} input_image: {input_image}')
...
class WanVideoUnit_ImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, input_image, end_image, height, width):
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, input_image, latents, height, width, tiled, tile_size, tile_stride):
        ...
        # print(f'DBG VAE: {self.__class__.__name__}')
        # print(f'DBG VAE: {self.__class__.__name__} input_image: {input_image[0].size}, latents: {latents.shape}, height: {height}, width: {width},  tiled:{tiled}, tile_size:{tile_size}, tile_stride:{tile_stride}')
...
class WanVideoUnit_FunControl(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, control_video, num_frames, height, width, tiled, tile_size, tile_stride, clip_feature, y, latents):
...
class WanVideoUnit_FunReference(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, reference_image, height, width):
...
class WanVideoUnit_FunCameraControl(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, height, width, num_frames, camera_control_direction, camera_control_speed, camera_control_origin, latents, input_image, tiled, tile_size, tile_stride):
...
class WanVideoUnit_SpeedControl(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, motion_bucket_id):
...
class WanVideoUnit_VACE(PipelineUnit):
    ...
    def process(
        self,
        pipe: KairosEmbodiedPipeline_DMD,
...
class WanVideoUnit_UnifiedSequenceParallel(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD):
...
class WanVideoUnit_TeaCache(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, num_inference_steps, tea_cache_l1_thresh, tea_cache_model_id):
...
class WanVideoUnit_CfgMerger(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, inputs_shared, inputs_posi, inputs_nega):
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_audio(self, pipe: KairosEmbodiedPipeline_DMD, input_audio, audio_sample_rate, num_frames, fps=16, audio_embeds=None, return_all=False):
    ...
    def process_motion_latents(self, pipe: KairosEmbodiedPipeline_DMD, height, width, tiled, tile_size, tile_stride, motion_video=None):
        pipe.load_models_to_device(["vae"])
        motion_frames = 73
        kwargs = {}
    ...
    def process_pose_cond(self, pipe: KairosEmbodiedPipeline_DMD, s2v_pose_video, num_frames, height, width, tiled, tile_size, tile_stride, s2v_pose_latents=None, num_repeats=1, return_all=False):
    ...
    @staticmethod
    def pre_calculate_audio_pose(pipe: KairosEmbodiedPipeline_DMD, input_audio=None, audio_sample_rate=16000, s2v_pose_video=None, num_frames=81, height=448, width=832, fps=16, tiled=True, tile_size=(30, 52), tile_stride=(15, 26)):
...
class WanVideoPostUnit_S2V(PipelineUnit):
    ...
    def process(self, pipe: KairosEmbodiedPipeline_DMD, latents, motion_latents, drop_motion_frames):
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # debug info 
    # now_shape = x.shape
    # # print('raw shape {} now shape: {}'.format(raw_shape, now_shape))
