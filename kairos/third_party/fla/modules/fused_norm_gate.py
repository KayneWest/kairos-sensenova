...
def layer_norm_gated_bwd(
    ...
    if weight is not None:
        ...
        if dist.is_initialized():
            dist.all_reduce(dw, op=dist.ReduceOp.SUM, group=tp_group)
    else:
        dw = None
    ...
    if bias is not None:
        ...
        if dist.is_initialized():
            dist.all_reduce(db, op=dist.ReduceOp.SUM, group=tp_group)
    else:
        db = None

    # print(f"mannually synced dw and db, group size = {dist.get_world_size(group=tp_group)}")
...
class LayerNormGatedFunction(torch.autograd.Function):
    ...
    def backward(ctx, dy, *args):
        ...
        dx, dg, dw, db, dres_in = layer_norm_gated_bwd(
            ...
            x_dtype=ctx.x_dtype,
            tp_group= getattr(ctx, "tp_group", None),
        )
        return (
