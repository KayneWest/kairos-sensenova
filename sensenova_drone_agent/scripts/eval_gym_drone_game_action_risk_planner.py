...
from sensenova_drone.action_risk_model import goal_features_from_info, load_action_risk_planner_runner
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate an action-conditioned risk planner in the Gym drone game.")
    ...
    parser.add_argument("--hover-penalty", type=float, default=None)
    parser.add_argument("--yaw-penalty", type=float, default=None)
...
def main() -> int:
    ...
    runner = load_action_risk_planner_runner(
        args.checkpoint,
        device=args.device,
    ...
    top_action_counts: Counter[str] = Counter()
    ...
    for episode_idx in range(args.episodes):
        ...
        top_collision_risks: list[float] = []
        top_clearances: list[float] = []
        top_utilities: list[float] = []

        while not done:
            if episode_idx < args.trace_episodes and len(trace_frames) < args.trace_frames:
                trace_frames.append(obs["image"])
            decision = runner.predict(
                Image.fromarray(obs["image"]),
                goal_features=goal_features_from_info(info),
                enabled_actions=enabled_actions,
            )
            top = decision.candidates[0]
            top_action_counts[top.action.value] += 1
            top_collision_risks.append(top.collision_risk)
            top_clearances.append(top.front_clearance_m)
            top_utilities.append(top.utility)
            obs, reward, terminated, truncated, info = env.step(decision.action_index)
            action_name = ACTION_VOCAB[decision.action_index]
        ...
        episodes.append(
            {
                ...
                "mean_top_collision_risk": mean(top_collision_risks),
                "mean_top_front_clearance_m": mean(top_clearances),
                "mean_top_utility": mean(top_utilities),
...
def summarize(
    *,
    ...
    return {
        ...
        "top_action_counts": dict(top_action_counts),
...
<head>
  <meta charset="utf-8">
  <title>Action-Risk Planner Eval</title>
  <style>
...
<body>
  <h1>Action-Risk Planner Eval</h1>
  <table>
